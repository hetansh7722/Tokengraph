...
- **Path traversal protection**: `repo_root` parameter now validates that the target directory contains a `.git` or `.tokengraph` directory, preventing arbitrary file exfiltration via MCP tools
...
- **Database path leakage**: Improved `.gitignore` inside `.tokengraph/` with explicit warnings about absolute paths in the database
...
- **Removed unused dependency**: `gitpython` was listed in dependencies but never imported â€” removed to shrink install footprint
- **Stale Docker reference**: Removed non-existent Docker image suggestion from Python version check

## [1.7.0] - 2026-03-09

### Added
- **`install` command** â€” primary entry point for new users (`tokengraph install`). `init` remains as an alias for backwards compatibility.
- **`--dry-run` flag** on `install`/`init` â€” shows what would be written without modifying files
- **PyPI publish workflow** â€” GitHub releases now automatically publish to PyPI via API token
- **Professional README** â€” complete rewrite with real benchmark data:
...
### Changed
- **Portable MCP config**: `init` now generates `uvx`-based `.mcp.json` instead of absolute Python paths â€” works on any machine with `uv` installed
- Removed `_safe_path` symlink workaround (no longer needed with `uvx`)

## [1.6.3] - 2026-03-06

### Added
- **SessionStart hook** â€” Claude Code now automatically prefers graph MCP tools over full codebase scans at the start of every session, saving tokens on general queries
...
### Fixed
- **Critical**: Incremental hash comparison bug â€” `file_hash` read from wrong field, causing every file to re-parse
...
### Added
- **Parser: C/C++ support** â€” full node extraction (structs, classes, functions, includes, calls, inheritance)
...
### Added
- **File organization**: All generated files now live in `.tokengraph/` directory instead of repo root
  - Auto-created `.gitignore` inside the directory prevents accidental commits
  - Automatic migration from legacy `.tokengraph.db` at repo root
...
### Changed
- Database path: `.tokengraph.db` â†’ `.tokengraph/graph.db`
- HTML visualization path: `.tokengraph.html` â†’ `.tokengraph/graph.html`
- `.tokengraph/**` added to default ignore patterns (prevents self-indexing)
...
### Added
- Universal installation: now works with `pip install tokengraph[embeddings]` on Python 3.10+
- CLI entry point (`tokengraph` command works after normal pip install)
...
### Changed
- Minimum Python requirement lowered from 3.11 â†’ 3.10 (covers ~90% of users)
