...
from tokengraph.registry import ConnectionPool, Registry, resolve_repo
...
class TestRegistry:
    ...
    def test_register_not_a_repo(self):
        """Registering a dir without .git or .tokengraph raises ValueError."""
        import pytest
...
class TestConnectionPool:
    ...
    def test_lru_ordering(self):
        ...
        # Now add db4 â€” db2 should be evicted (LRU), not db1
        self.pool.get(db4)
        assert self.pool.size == 3
...
class TestCrossRepoSearch:
    def test_cross_repo_search_no_repos(self):
        ...
        from tokengraph.tools import cross_repo_search_func

        tmp_dir = tempfile.mkdtemp()

        with patch("tokengraph.registry.Registry") as mock_registry_cls:
