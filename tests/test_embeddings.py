...
from tokengraph.embeddings import (
    LOCAL_DEFAULT_MODEL,
    EmbeddingStore,
...
class TestEmbeddingStore:
    def test_store_initializes(self, tmp_path):
        ...
        with patch("tokengraph.embeddings.get_provider", return_value=None):
...
class TestGetProviderModel:
    ...
    @patch("tokengraph.embeddings.LocalEmbeddingProvider")
...
class TestEmbeddingStoreModelPassthrough:
    ...
    def test_model_forwarded_to_get_provider(self, tmp_path):
        db = tmp_path / "embeddings.db"
        with patch("tokengraph.embeddings.get_provider", return_value=None) as mock_gp:
