...
from tokengraph.graph import GraphStore, _sanitize_name, node_to_dict
from tokengraph.parser import EdgeInfo, NodeInfo
from tokengraph.tools import (
...
class TestSanitizeName:
    ...
    def test_node_to_dict_uses_sanitize(self):
        ...
        from tokengraph.graph import GraphNode
        node = GraphNode(
...
class TestFlowTools:
    ...
    def setup_method(self):
        """Set up a temp dir with .git and .tokengraph, seed data, build flows."""
        ...
        (self.root / ".git").mkdir()
        (self.root / ".tokengraph").mkdir()

        db_path = str(self.root / ".tokengraph" / "graph.db")
...
class TestCommunityTools:
    ...
    def setup_method(self):
        """Set up a temp dir with .git and .tokengraph, seed clustered graph."""
        self.tmp_dir = tempfile.mkdtemp()
        self.root = Path(self.tmp_dir).resolve()
    ...
    def _build_communities(self):
        ...
        from tokengraph.communities import detect_communities, store_communities
