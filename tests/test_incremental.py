...
from tokengraph.incremental import (
    _is_binary,
...
class TestGetDbPath:
    def test_creates_directory_and_db_path(self, tmp_path):
        ...
        assert db_path == tmp_path / ".tokengraph" / "graph.db"
        assert (tmp_path / ".tokengraph").is_dir()

    def test_creates_gitignore(self, tmp_path):
        get_db_path(tmp_path)
        gi = tmp_path / ".tokengraph" / ".gitignore"
        assert gi.exists()
        assert "*\n" in gi.read_text()

    def test_migrates_legacy_db(self, tmp_path):
        legacy = tmp_path / ".tokengraph.db"
    ...
    def test_cleans_legacy_side_files(self, tmp_path):
        ...
        for suffix in ("-wal", "-shm", "-journal"):
            (tmp_path / f".tokengraph.db{suffix}").write_text("side")
        get_db_path(tmp_path)
        for suffix in ("-wal", "-shm", "-journal"):
            assert not (tmp_path / f".tokengraph.db{suffix}").exists()
...
class TestGitOperations:
    @patch("tokengraph.incremental.subprocess.run")
...
class TestFullBuild:
    def test_full_build_parses_files(self, tmp_path):
        ...
        try:
            mock_target = "tokengraph.incremental.get_all_tracked_files"
