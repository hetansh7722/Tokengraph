...
from tokengraph.changes import (
    _parse_unified_diff,
    analyze_changes,
    compute_risk_score,
...
from tokengraph.flows import store_flows, trace_flows
...
class TestChanges:
    ...
    def test_detect_changes_tool_no_changes(self):
        ...
        from tokengraph.tools import detect_changes_func

        # Patch _get_store to use our test store,
        # and get_changed_files/get_staged_and_unstaged to return empty.
        with (
            patch("tokengraph.tools.review._get_store") as mock_get_store,
            patch("tokengraph.tools.review.get_changed_files", return_value=[]),
            patch("tokengraph.tools.review.get_staged_and_unstaged", return_value=[]),
        ):
    ...
    def test_detect_changes_tool_with_changes(self):
        ...
        with (
            patch("tokengraph.tools.review._get_store") as mock_get_store,
            patch("tokengraph.tools.review.get_changed_files", return_value=["app.py"]),
            patch(
                "tokengraph.tools.review.parse_git_diff_ranges",
