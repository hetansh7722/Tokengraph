    ...
    for n in nodes:
        ...
        if fp:
            ...
            if n["kind"] == "File":
                ...
                # "Foo.hpp", "trading/Foo.hpp", "libs/trading/Foo.hpp", â€¦
...
def _resolve_target(
    ...
    # Ambiguous â€” pick first match rather than dropping the edge
    return candidates[0]
...
def export_graph_data(store: GraphStore) -> dict:
    ...
    try:
        from tokengraph.flows import get_flows
    ...
    try:
        from tokengraph.communities import get_communities
...
def generate_html(store: GraphStore, output_path: str | Path) -> Path:
    ...
    if stats.total_nodes > 50000:
        logger.warning(
            "Graph has %d nodes â€” visualization may be slow. "
