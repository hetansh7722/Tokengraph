...
Usage:
    tokengraph install
    tokengraph init
    tokengraph build [--base BASE]
    tokengraph update [--base BASE]
    tokengraph watch
    tokengraph status
    tokengraph serve
    tokengraph visualize
    tokengraph wiki
    tokengraph detect-changes [--base BASE] [--brief]
    tokengraph register <path> [--alias name]
    tokengraph unregister <path_or_alias>
    tokengraph repos
"""
...
# Python version check â€” must come before any other imports
if sys.version_info < (3, 10):
    print("tokengraph requires Python 3.10 or higher.")
...
def _print_banner() -> None:
    ...
    # ANSI escape codes
    c = "\033[36m" if color else ""   # cyan â€” graph art
    y = "\033[33m" if color else ""   # yellow â€” center node
    b = "\033[1m" if color else ""    # bold
    d = "\033[2m" if color else ""    # dim
    g = "\033[32m" if color else ""   # green â€” commands
    r = "\033[0m" if color else ""    # reset

    print(f"""
{c}  â—â”€â”€â—â”€â”€â—{r}
{c}  â”‚â•² â”‚ â•±â”‚{r}       {b}tokengraph{r}  {d}v{version}{r}
{c}  â—â”€â”€{y}â—†{c}â”€â”€â—{r}
{c}  â”‚â•± â”‚ â•²â”‚{r}       {d}Structural knowledge graph for{r}
{c}  â—â”€â”€â—â”€â”€â—{r}       {d}smarter code reviews{r}
  ...
  {d}Run{r} {b}tokengraph <command> --help{r} {d}for details{r}
""")
...
def _handle_init(args: argparse.Namespace) -> None:
    ...
    print()
    print("Next steps:")
    print("  1. tokengraph build    # build the knowledge graph")
...
def main() -> None:
    ...
    if args.version:
        print(f"tokengraph {_get_version()}")
        return
    ...
    if args.command == "eval":
        ...
        if getattr(args, "report", False):
            ...
            print("Run 'tokengraph eval --report' to generate tables.")
        return
    ...
    if args.command in ("register", "unregister", "repos"):
        ...
        if args.command == "register":
            ...
            if not repos:
                ...
                print("Use: tokengraph register <path> [--alias name]")
            else:
                for entry in repos:
    ...
    try:
        if args.command == "build":
            ...
            html_path = repo_root / ".tokengraph" / "graph.html"
            ...
            wiki_dir = repo_root / ".tokengraph" / "wiki"
