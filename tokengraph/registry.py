...
_REGISTRY_DIR = Path.home() / ".tokengraph"
_REGISTRY_PATH = _REGISTRY_DIR / "registry.json"


class Registry:
    """Manages a JSON-based registry of tokengraph repositories.

    Each entry stores the repo path and an optional alias.
    The registry lives at ``~/.tokengraph/registry.json``.
    """
    ...
    def register(self, path: str, alias: str | None = None) -> dict[str, str]:
        ...
        Validates that the path contains a ``.git`` or ``.tokengraph``
        directory.
        ...
        if not (resolved / ".git").exists() and not (resolved / ".tokengraph").exists():
            raise ValueError(
                f"Path does not look like a repository "
                f"(no .git or .tokengraph): {resolved}"
            )
