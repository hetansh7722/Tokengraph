...
def get_db_path(repo_root: Path) -> Path:
    ...
    Creates the ``.tokengraph/`` directory and an inner ``.gitignore``
    (with ``*``) so generated files are never committed.  If a legacy
    ``.tokengraph.db`` exists at the repo root the database is migrated
    into the new directory (WAL/SHM side-files are discarded).
    """
    crg_dir = repo_root / ".tokengraph"
...
def _load_ignore_patterns(repo_root: Path) -> list[str]:
    """Load ignore patterns from .tokengraphignore file."""
    patterns = list(DEFAULT_IGNORE_PATTERNS)
    ignore_file = repo_root / ".tokengraphignore"
