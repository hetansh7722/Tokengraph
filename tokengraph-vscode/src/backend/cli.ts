...
export class CliWrapper {
    ...
    async isInstalled(): Promise<boolean> {
        try {
            ...
            // Non-zero exit or other transient error â€” treat as not installed.
            return false;
        }
    }
    ...
    async detectPythonInstaller(): Promise<'uv' | 'pipx' | 'pip' | null> {
        ...
        for (const { bin, result } of candidates) {
            try {
                ...
                // Not found or errored â€” try next.
            }
        }
    ...
    /**
     * Install the `tokengraph` package using the specified installer.
     */
    async installBackend(installer: 'uv' | 'pipx' | 'pip'): Promise<CliResult> {
        const commandMap: Record<typeof installer, { bin: string; args: string[] }> = {
            uv: { bin: 'uv', args: ['pip', 'install', 'tokengraph'] },
            pipx: { bin: 'pipx', args: ['install', 'tokengraph'] },
            pip: { bin: 'pip3', args: ['install', 'tokengraph'] },
        };
...
function toCliResult(err: unknown): CliResult {
    ...
    if (isEnoent(err)) {
        return {
            success: false,
            stdout: '',
            stderr: 'CLI binary not found. Is tokengraph installed?',
        };
    }
