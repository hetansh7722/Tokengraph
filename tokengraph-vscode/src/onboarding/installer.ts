 ...
 *
 * Checks whether the `tokengraph` CLI is available and, if not,
...
export class Installer {
    ...
    async checkAndPrompt(): Promise<boolean> {
        ...
        if (selection === 'Manual Instructions') {
            ...
            terminal.sendText('echo "  pip install tokengraph"');
            terminal.sendText('echo ""');
            terminal.sendText('echo "Option 2: Install with pipx (recommended)"');
            terminal.sendText('echo "  pipx install tokengraph"');
            terminal.sendText('echo ""');
    ...
    async autoInstall(): Promise<boolean> {
        ...
        await vscode.window.withProgress(
            {
                ...
                title: `Installing tokengraph via ${installer}...`,
                cancellable: false,
            },
            async () => {
                try {
        ...
        vscode.window.showErrorMessage(
            `Failed to install tokengraph via ${installer}. ` +
            'Check the terminal output for details or try installing manually: ' +
            `\`${installer} install tokengraph\``,
        );
