 ...
 * Checks `.tokengraph/graph.db` first, then falls back to `.tokengraph.db`.
 */
function findGraphDb(workspaceRoot: string): string | undefined {
  const primary = path.join(workspaceRoot, ".tokengraph", "graph.db");
  if (fs.existsSync(primary)) {
    return primary;
  }

  const fallback = path.join(workspaceRoot, ".tokengraph.db");
...
function registerCommands(
  ...
  // -----------------------------------------------------------------
  // codeReviewGraph.queryGraph â€” expose all 8 query patterns
  // -----------------------------------------------------------------
  context.subscriptions.push(
    vscode.commands.registerCommand("codeReviewGraph.queryGraph", async () => {
      ...
      if (!node) {
        ...
        if (matches.length === 1) {
          ...
          const selected = await vscode.window.showQuickPick(
            matches.map(m => ({
              label: m.name,
              description: `${m.kind} Â· ${m.filePath}`,
              node: m,
            })),
            { placeHolder: `Multiple matches for "${target}" â€” select one` },
          );
          if (!selected) { return; }
          node = selected.node;
        }
      ...
      const items = filtered.map(e => {
        ...
        return {
          ...
          description: relatedNode ? `${relatedNode.kind} Â· ${relatedNode.filePath}` : "",
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        const items = results.map(r => ({
          ...
          description: `${r.kind} Â· ${r.filePath}`,
          detail: `Lines ${r.lineStart ?? "?"}â€“${r.lineEnd ?? "?"}`,
          node: r as GraphNode,
        }));
    ...
    vscode.commands.registerCommand("codeReviewGraph.embedGraph", async () => {
      ...
      if (result.success) {
        ...
        const msg = result.stderr.includes("not installed")
          ? "Install embeddings support: pip install tokengraph[embeddings]"
        ...
        await vscode.window.withProgress(
          ...
          async () => {
            ...
            if (impactedFileCount > 10) {
              guidance.push(
                `\u26a0\ufe0f **Wide blast radius**: ${impactedFileCount} files impacted â€” consider splitting this change.`
              );
            }
...
export async function activate(
  ...
  // Register revealInTree command for bidirectional graphâ†’tree sync
  context.subscriptions.push(
    vscode.commands.registerCommand(
