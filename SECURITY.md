...
tokengraph is a **local development tool**. It:
- Runs as a local MCP server via stdio (no network listener)
- Stores data in a local SQLite database (`.tokengraph/graph.db`)
...
| Path Traversal | `_validate_repo_root()` requires `.git` or `.tokengraph` directory |
