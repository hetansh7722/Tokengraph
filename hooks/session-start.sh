...
DB_PATH=".tokengraph/graph.db"

if [ -f "$DB_PATH" ]; then
    cat <<'INSTRUCTIONS'
[tokengraph] Knowledge graph is available.

When answering questions about this codebase, prefer using the tokengraph MCP tools before scanning files manually:
...
INSTRUCTIONS
    ...
    echo "[tokengraph] No knowledge graph found. Run /tokengraph:build-graph to parse this codebase and enable graph-powered queries."
fi
