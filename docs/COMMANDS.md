...
```
Requires: `pip install tokengraph[embeddings]`
...
```bash
# Setup
tokengraph install           # Register MCP server with Claude Code (alias: init)
tokengraph install --dry-run # Preview without writing files

# Build and update
tokengraph build                        # Full build
tokengraph update                       # Incremental update
tokengraph update --base origin/main    # Custom base ref
...
# Analysis
tokengraph detect-changes               # Risk-scored change analysis
tokengraph detect-changes --base HEAD~3 # Custom base ref
tokengraph detect-changes --brief       # Compact output

# Wiki
tokengraph wiki                         # Generate markdown wiki from communities

# Multi-repo
tokengraph register <path> [--alias name]  # Register a repository
tokengraph unregister <path_or_alias>       # Remove from registry
...
# Server
tokengraph serve                        # Start MCP server (stdio)
```
