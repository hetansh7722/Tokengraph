...
<section name="usage">
Quick install: pip install tokengraph
Then: tokengraph install && tokengraph build
First run: /tokengraph:build-graph
...
CLI: tokengraph [install|init|build|update|status|watch|visualize|serve|wiki|detect-changes|register|unregister|repos|eval]
</section>

<section name="legal">
MIT license. 100% local. No telemetry. DB file: .tokengraph/graph.db
</section>

<section name="watch">
Run: tokengraph watch (auto-updates graph on file save via watchdog)
Or use PostToolUse (Write|Edit|Bash) hooks for automatic background updates.
</section>

<section name="embeddings">
Optional: pip install tokengraph[embeddings]
...
Large repos: First build 30-60s. Incremental <2s. Add patterns to .tokengraphignore.
Stale graph: Run /tokengraph:build-graph manually.
...
When user asks anything about "tokengraph", "how to use", "commands", "review-delta", etc.:
