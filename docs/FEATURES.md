...
## v1.7.0
- **`install` command**: New primary entry point for setup (`tokengraph install`). `init` remains as an alias.
...
## v1.6.4
- **Portable MCP config**: `init` now generates `uvx`-based `.mcp.json` â€” no absolute paths, works on any machine with `uv` installed
...
- **File organization**: Generated files moved into `.tokengraph/` directory (auto-created `.gitignore`, legacy migration)
...
- **Interactive D3.js graph visualization**: `tokengraph visualize` generates an HTML graph you can explore in-browser
...
- **Universal install**: `pip install tokengraph` â€” no git clone needed
- **CLI entry point**: `tokengraph` command available system-wide after pip install
...
## v1.1.0
- **Watch mode**: `tokengraph watch` â€” auto-rebuilds graph on file changes
...
## v1.0.0 (Foundation)
- **Persistent SQLite knowledge graph** â€” zero external dependencies
- **Tree-sitter multi-language parsing** â€” classes, functions, imports, calls, inheritance
- **Incremental updates** via `git diff` + automatic dependency cascade
- **Impact-radius / blast-radius analysis** â€” BFS through call/import/inheritance graph
...
- Graph stored in `.tokengraph/graph.db` (SQLite), auto-gitignored
- No telemetry, no network calls
- Respects `.gitignore` and `.tokengraphignore`
