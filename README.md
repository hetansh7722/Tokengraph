...
<p align="center">
  <a href="https://github.com/hetansh7722/tokengraph/stargazers"><img src="https://img.shields.io/github/stars/hetansh7722/tokengraph?style=flat-square" alt="Stars"></a>
  <a href="https://opensource.org/licenses/MIT"><img src="https://img.shields.io/badge/License-MIT-yellow.svg?style=flat-square" alt="MIT Licence"></a>
  <a href="https://github.com/hetansh7722/tokengraph/actions/workflows/ci.yml"><img src="https://github.com/hetansh7722/tokengraph/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
...
Claude Code re-reads your entire codebase on every task. `tokengraph` fixes that. It builds a structural map of your code with [Tree-sitter](https://tree-sitter.github.io/tree-sitter/), tracks changes incrementally, and gives Claude precise context so it reads only what matters.
...
tokengraph install          # auto-detects and configures all supported platforms
...
All numbers come from the automated evaluation runner against 6 real open-source repositories (13 commits total). Reproduce with `tokengraph eval --all`. Raw data in [`evaluate/reports/summary.md`](evaluate/reports/summary.md).
...
The blast-radius analysis never misses an actually impacted file (perfect recall). It over-predicts in some cases, which is a conservative trade-off â€” better to flag too many files than miss a broken dependency.
...
| Command | Description |
|---------|-------------|
| `/tokengraph:build-graph` | Build or rebuild the code graph |
| `/tokengraph:review-delta` | Review changes since last commit |
| `/tokengraph:review-pr` | Full PR review with blast-radius analysis |
...
```bash
tokengraph install          # Auto-detect and configure all platforms
tokengraph install --platform <name>  # Target a specific platform
tokengraph build            # Parse entire codebase
tokengraph update           # Incremental update (changed files only)
tokengraph status           # Graph statistics
tokengraph watch            # Auto-update on file changes
tokengraph visualize        # Generate interactive HTML graph
tokengraph wiki             # Generate markdown wiki from communities
tokengraph detect-changes   # Risk-scored change impact analysis
tokengraph register <path>  # Register repo in multi-repo registry
tokengraph unregister <id>  # Remove repo from registry
tokengraph repos            # List registered repositories
tokengraph eval             # Run evaluation benchmarks
tokengraph serve            # Start MCP server
```
...
To exclude paths from indexing, create a `.tokengraphignore` file in your repository root:
...
```bash
pip install tokengraph[embeddings]          # Local vector embeddings (sentence-transformers)
pip install tokengraph[google-embeddings]   # Google Gemini embeddings
pip install tokengraph[communities]         # Community detection (igraph)
pip install tokengraph[eval]                # Evaluation benchmarks (matplotlib)
pip install tokengraph[wiki]                # Wiki generation with LLM summaries (ollama)
pip install tokengraph[all]                 # All optional dependencies
```
...
Edit `tokengraph/parser.py` and add your extension to `EXTENSION_TO_LANGUAGE` along with node type mappings in `_CLASS_TYPES`, `_FUNCTION_TYPES`, `_IMPORT_TYPES`, and `_CALL_TYPES`. Include a test fixture and open a PR.
...
<p align="center">
<br>
<code>pip install tokengraph && tokengraph install</code><br>
